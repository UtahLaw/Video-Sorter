...
The custom client deliberately uploads through `https://www.kaltura.com/api_v3/service/uploadtoken/action/upload`, matching the repository's proven legacy behavior. It does not use the alternate `uploadUrl` host returned with a token: in production, that host acknowledged the bytes without advancing the token from PENDING. The client forces `format=1` on both API and file-upload calls. Kaltura's upload endpoint can acknowledge accepted bytes with HTTP 202 and an empty body; in that case the client polls `uploadToken.get` and continues only after the token reaches full-upload status. It URL-encodes query values and applies bounded connect/read timeouts. Failures identify the upload stage, HTTP status, content type, sanitized endpoint, and response size without logging session tokens or query parameters.
